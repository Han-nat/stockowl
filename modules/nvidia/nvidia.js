                ...
                for (let i = 0; i < this.products.length; i++)
                {
                    try
                    {
                        ...
                        if (BUYNOW)
                        {
                            ...
                            let priceSpans = await page.$$('div.price > span')
                            let price = -1
    
                            for (let j = 0; j < priceSpans.length; j++)
                            {
                                let text = await priceSpans[j].evaluate(span => span.innerText)
                                if (text.includes('£'))
                                {
